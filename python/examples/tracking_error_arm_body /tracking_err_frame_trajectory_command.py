...
data_file = "tracking_data_forward_1.dat"
    ...
    with LeaseKeepAlive(lease_client, must_acquire=True, return_at_exit=True):
        ...
        try:
            return relative_move(options.dx, options.dy, math.radians(options.dyaw), options.frame,
                                 robot_command_client, robot_state_client, data ,stairs=options.stairs)
        finally:
            # Send a Stop at the end, regardless of what happened.
            robot_command_client.robot_command(RobotCommandBuilder.stop_command())


def relative_move(dx, dy, dyaw, frame_name, robot_command_client, robot_state_client, data, stairs=False):
    ...
    while True:
        ...
        # print(feedback)
        # print(mobility_feedback)
        block_until_done_future = robot_state_client.get_robot_state_async()
        try:
            data.encoder_traj.robot_state.append(block_until_done_future.result(timeout=.1).kinematic_state)
