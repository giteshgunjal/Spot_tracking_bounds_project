...
def main():
    ...
    def async_callback(results):
        ...
        nonlocal check3
        print("callback", time.perf_counter()-check3)
    ...
    print(type(check_until_done_future.result()))
    check4 = time.perf_counter()
    
    data.encoder_traj.robot_state.append(check_until_done_future.result().kinematic_state)
    print("check", time.perf_counter()-check4)
    ...
    try:
        ...
        print("block", time.perf_counter()-check1)
    except TimeoutError:
